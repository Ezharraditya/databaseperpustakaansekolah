...
--
-- Host: 127.0.0.1
-- Generation Time: Feb 24, 2025 at 03:04 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `DaftarSeluruhSiswa` ()   BEGIN
    SELECT s.id_siswa, s.nama_siswa, s.kelas, 
           IFNULL(COUNT(p.id_peminjaman), 0) AS total_peminjaman
    FROM siswa s
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `DaftarSiswaPeminjam` ()   BEGIN
    SELECT DISTINCT s.id_siswa, s.nama_siswa, s.kelas
    FROM siswa s
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_buku` (IN `pJudulBuku` VARCHAR(255), IN `pPenulis` VARCHAR(50), IN `pKategori` VARCHAR(30), IN `pStok` INT)   BEGIN
